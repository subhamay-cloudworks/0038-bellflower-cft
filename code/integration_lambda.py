...
def write_to_s3(bucket_name, execution_id, file_name, data):
    ...
    try:
        s3_object = s3_resource.Object(
            bucket_name, f"outbound/{execution_id}/{file_name}")
...
def lambda_handler(event, context):
    ...
    logger.info(
        f"function_name = {context.function_name} : event_source = {get_event_source(event)}")
    try:
        if get_event_source(event) == "sqs":
            ...
            response = write_to_s3(
                s3_bucket_name, execution_id, 'task_token.txt', task_token)

            if response == '200':
                logger.info(
                    f"task_token uploaded to S3 bucket outbound folder successfully !")

            response = write_to_s3(
                s3_bucket_name, execution_id, 'data.json', json.dumps(info))
            if response == '200':
                logger.info(
                    f"info uploaded to S3 bucket outbound folder successfully !")
            ...
            logger.info(f"execution_id = {execution_id}")
            task_token = read_from_s3(
                s3_bucket_name, f"outbound/{execution_id}/task_token.txt")
